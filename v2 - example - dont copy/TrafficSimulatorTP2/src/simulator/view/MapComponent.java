...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			int x2 = r.getCruceDest().getX();
			int y2 = r.getCruceDest().getY();

			// choose a color for the arrow depending on the traffic light of the road
			Color arrowColor = _RED_LIGHT_COLOR;
			int idx = r.getCruceDest().getGreenLightIndex();
			if (idx != -1 && r.equals(r.getCruceDest().getInRoads().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getTotalCO2() / (1.0 + (double) r.getAlarContExcesiva())));
	...
	private void drawVehicles(Graphics g) {
		for (Vehicle v : _map.getVehicles()) {
			if (v.getEstado() != VehicleStatus.ARRIVED) {
				...
				// contamination class
				int vLabelColor = (int) (25.0 * (10.0 - (double) v.getGradCont()));
